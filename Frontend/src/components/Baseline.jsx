...
const Baseline = () => {
  ...
  useEffect(() => {
    ...
    const chart = createChart(barChart.current, {
      layout: {
        textColor: "black",
    ...
    // --- data ---
    const data = ([{ time: '2018-03-28', value: 154 },
    ...
    toolTip.style = `
      width: 120px; height: auto; position: absolute; display: none;
    ...
    `;
    barChart.current.appendChild(toolTip);
    ...
    chart.subscribeCrosshairMove((param) => {
      if (
        ...
        !param.point ||
        param.point.x < 0 ||
        param.point.x > barChart.current.clientWidth ||
        param.point.y < 0 ||
        param.point.y > barChart.current.clientHeight
      ) {
        toolTip.style.display = "none";
        return;
      }

      const dataAtPoint = param.seriesData.get(baselineSeries);
      const price = dataAtPoint?.value ?? dataAtPoint?.close;
      if (price === undefined) {
        toolTip.style.display = "none";
        return;
      }
      ...
      toolTip.innerHTML = `
        ...
        <div style="font-size: 20px; margin: 4px 0; color: black">
      ...
      if (left > barChart.current.clientWidth - 100) {
        left = param.point.x - 120;
      }

      let top = param.point.y + 15;
      if (top > barChart.current.clientHeight - 80) {
        top = param.point.y - 90;
      }
  ...
  return <div ref={barChart} style={{ width: "100%", height: "500px" }} />;
};
