...
const AllBots = () => {
  ...
  useEffect(() => {
    ...
    // Set up polling to refresh bots list every 30 seconds (less frequent for AllBots)
  ...
  // Filter and sort bots - ALL bots (active and inactive)
  const filteredAndSortedBots = useMemo(() => {
    // Filter: show ALL bots except hidden ones (no active/running filter)
    let filtered = bots.filter(bot => !hiddenBots.has(bot.id));
    ...
    <div className="page-container">
      <div className="current-bots-header">
        <div>
          ...
          <p className="page-description">View all trading bots (active and inactive)</p>
        </div>
                    ...
                    <td>
                      <span className={`status-badge ${bot.status?.toLowerCase() || 'active'}`}>
                        {bot.status === 'ACTIVE' && bot.is_running ? 'Running' : 
                         ...
                         bot.status === 'HARD_STOPPED_OUT' ? 'Stopped' :
                         bot.status === 'STOPPED' ? 'Stopped' :
                         !bot.is_active ? 'Inactive' :
                         !bot.is_running ? 'Paused' : 'Not started'}
                      </span>
                    </td>
