...
if [[ -f "${IBC_PATH}/config.ini.template" ]]; then
    echo "Copying config.ini template..."
    cp "${IBC_PATH}/config.ini.template" "${IBC_INI}"
fi

# Inject credentials from environment variables if present
if [[ -f "${IBC_PATH}/scripts/inject_credentials.sh" ]]; then
    echo "Injecting credentials from environment variables..."
    bash "${IBC_PATH}/scripts/inject_credentials.sh"
fi

# Start 2FA handler in background
if [[ -f "${IBC_PATH}/scripts/handle_2fa.sh" ]]; then
    echo "Starting 2FA handler..."
    bash "${IBC_PATH}/scripts/handle_2fa.sh" &
fi
